#!/usr/bin/env python3
"""
IMU Data Plotter - Live visualization of Roll, Pitch, Heading, Yaw Rate
...
# Configuration
BUFFER_SIZE = 200       # Number of data points to display
...
class IMUPlotter:
    def __init__(self, port):
        ...
        self.fig.suptitle('IMU Orientation + Yaw Rate', fontsize=14)
        ...
        self.ax3.set_title('Yaw Rate (Angular Velocity)')

        # Magnetometer subplot (User coordinates: X=forward, Y=left, Z=up/USB)
        self.ax4.set_xlabel('Samples')
        self.ax4.set_ylabel('milli-Gauss')
        self.ax4.set_xlim(0, BUFFER_SIZE)
        self.ax4.grid(True, alpha=0.3)
        self.ax4.axhline(y=0, color='gray', linestyle='--', alpha=0.5)
        self.line_mag_x, = self.ax4.plot([], [], 'r-', label='X (forward)', linewidth=1.2)
        self.line_mag_y, = self.ax4.plot([], [], 'g-', label='Y (left)', linewidth=1.2)
        self.line_mag_z, = self.ax4.plot([], [], 'b-', label='Z (up/USB)', linewidth=1.2)
        self.ax4.legend(loc='upper right')
        self.ax4.set_title('Magnetometer (Raw)')
    ...
    def parse_line(self, line):
        ...
        try:
            ...
            if line.startswith('IMU,') and len(parts) >= 5:
                ...
                # Yaw rate is optional (new format)
                if len(parts) >= 6:
    ...
    def update_plot(self, frame):
        ...
        if len(self.yaw_rate_data) > 0:
            ...
            self.ax3.set_title(f'Yaw Rate: {latest_yaw_rate:+.1f} °/s')

            # Auto-scale Y axis for yaw rate
            yaw_min = min(self.yaw_rate_data)
            yaw_max = max(self.yaw_rate_data)
            margin = max(20, (yaw_max - yaw_min) * 0.1)
            self.ax3.set_ylim(min(-50, yaw_min - margin), max(50, yaw_max + margin))
        ...
        if len(self.mag_x) > 0:
            ...
            if all_mag:
                ...
                self.ax4.set_ylim(mag_min - margin, mag_max + margin)

            latest_mx = self.mag_x[-1]
            latest_my = self.mag_y[-1]
            latest_mz = self.mag_z[-1]

            self.ax4.set_title(f'Raw Magnetometer: X={latest_mx:+d}  Y={latest_my:+d}  Z={latest_mz:+d} mG')
        ...
        status = f"IMU: {self.imu_count} ({imu_rate:.1f} Hz)  |  MAG: {self.mag_count} ({mag_rate:.1f} Hz)"
        self.status_text.set_text(status)

        return (self.line_roll, self.line_pitch, self.line_heading, self.line_yaw_rate,
                self.line_mag_x, self.line_mag_y, self.line_mag_z, self.status_text)
...
def main():
    ...
    print("=" * 50)
    print("  IMU + Magnetometer Data Plotter")
    print("=" * 50)
    print(f"Port: {port}")
